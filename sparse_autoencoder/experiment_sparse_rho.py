"""
Rho (sparsity target) sweep for the sparse autoencoder.

This experiment trains the sparse autoencoder several times with different
SPARSITY_TARGET (rho) values while keeping every other setting identical
(same architecture, seed, epochs, learning rate, and sparsity weight beta).
rho is the desired average activation of each encoder unit: a small rho asks
each unit to stay mostly inactive (stronger sparsity), while a larger rho
allows more units to fire on average (weaker sparsity).

For each rho it calibrates a p95 threshold on the normal calibration split
and evaluates on the anomaly reference split, then records precision, recall,
F1, false-positive rate, and per-category recall.

The sparsity weight beta is fixed at the best value found in the beta sweep so
this experiment isolates the effect of the sparsity target alone.
...
# rho values to compare (from strong to weak sparsity)
RHO_VALUES = [0.01, 0.025, 0.05, 0.1, 0.2]

# sparsity weight fixed at the best value from the beta sweep
FIXED_BETA = 1e-3
...
RESULTS_CSV_PATH = EXPERIMENT_DIR / "sparse_rho_sweep.csv"
RESULTS_JSON_PATH = EXPERIMENT_DIR / "sparse_rho_sweep.json"
FIGURE_PATH = EXPERIMENT_DIR / "sparse_rho_sweep.png"


def set_seed() -> None:
    """
    Reset all random seeds so every rho run starts from the same state.
    """
    random.seed(SEED)
    np.random.seed(SEED)
...
def train_with_rho(
    ...
    """
    Train the sparse autoencoder for a single rho value.
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for inputs, targets in train_loader:
            ...
            penalty = sparsity_penalty(activations, rho)
...
def plot_results(rows: list[dict]) -> None:
    """
    Plot overall F1/precision/recall and per-category recall against rho.
    """
    ...
    axes[0].plot(rhos, [row["precision"] for row in rows], marker="o", label="Precision")
    axes[0].plot(rhos, [row["recall"] for row in rows], marker="o", label="Recall")
    axes[0].plot(rhos, [row["f1"] for row in rows], marker="o", label="F1")
    axes[0].set_xscale("log")
    axes[0].set_xticks(rhos)
    axes[0].set_xticklabels([f"{rho:g}" for rho in rhos])
    axes[0].set_xlabel("Sparsity target rho")
    axes[0].set_ylabel("Score (p95 threshold)")
    axes[0].set_title("Overall Metrics vs Rho")
    ...
    axes[1].set_xticklabels([f"{rho:g}" for rho in rhos])
    axes[1].set_xlabel("Sparsity target rho")
    axes[1].set_ylabel("Recall (p95 threshold)")
    axes[1].set_title("Per-Category Recall vs Rho")
    axes[1].grid(alpha=0.25)
    axes[1].legend()

    fig.suptitle(f"Sparse Autoencoder Rho Sweep (beta={FIXED_BETA:g})")
    fig.tight_layout()
...
def run_sweep() -> None:
    ...
    # load data once and reuse across all rho runs
    ...
    print(f"Fixed sparsity weight (beta): {FIXED_BETA}")
    print(f"Rho values: {RHO_VALUES}")

    rows = []
    for rho in RHO_VALUES:
        print(f"\n=== Training with rho={rho:g} ===")
        model, best_validation_reconstruction, final_validation_sparsity = train_with_rho(
            rho, FIXED_BETA, x_train, x_validation, device
        )
        metrics = evaluate_rho(
        ...
        rows.append(row)
        print(
            f"rho={rho:g}: precision={metrics['precision']:.4f}, "
    ...
    print("\n=== Rho sweep summary (p95) ===")
    header = f"{'rho':>8} {'precision':>10} {'recall':>8} {'f1':>8} {'fpr':>8}"
    print(header)
    for row in rows:
        print(
            f"{row['rho']:>8g} {row['precision']:>10.4f} {row['recall']:>8.4f} "

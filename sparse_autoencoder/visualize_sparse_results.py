"""
Visualize the sparse autoencoder results.

Produces the training curve, reconstruction error distribution, precision /
recall / F1 vs threshold, precision-recall curve, and confusion matrix.
Figures are saved under artifacts/figures/sparse_autoencoder so they do not
overwrite the baseline figures.
"""
...
from train_sparse_autoencoder import HISTORY_PATH


# paths
PROJECT_ROOT = Path(__file__).resolve().parents[1]
FIGURE_DIR = PROJECT_ROOT / "artifacts" / "figures" / "sparse_autoencoder"
FIGURE_DPI = 220
...
def save_training_curve() -> None:
    """
    Plot training and validation reconstruction error by epoch.

    The sparse training history also stores the sparsity penalty, which is
    plotted on a secondary axis so the two signals can be read together.
    """
    ...
    with HISTORY_PATH.open("r", encoding="utf-8") as file:
        ...
        for row in reader:
            ...
            train_reconstruction.append(float(row["train_reconstruction"]))
            validation_reconstruction.append(float(row["validation_reconstruction"]))
            validation_sparsity.append(float(row["validation_sparsity"]))

    fig, ax_reconstruction = plt.subplots(figsize=(7, 4.5))
    ax_reconstruction.plot(epochs, train_reconstruction, label="Training normal")
    ax_reconstruction.plot(epochs, validation_reconstruction, label="Validation normal")
    ax_reconstruction.set_xlabel("Epoch")
    ax_reconstruction.set_ylabel("Mean squared reconstruction error")

    # sparsity penalty on a secondary axis
    ax_sparsity = ax_reconstruction.twinx()
    ax_sparsity.plot(
        epochs,
        validation_sparsity,
        color="green",
        linestyle="--",
        label="Validation sparsity (KL)",
    )
    ax_sparsity.set_ylabel("KL sparsity penalty")

    # combine legends from both axes
    lines_1, labels_1 = ax_reconstruction.get_legend_handles_labels()
    lines_2, labels_2 = ax_sparsity.get_legend_handles_labels()
    ax_reconstruction.legend(lines_1 + lines_2, labels_1 + labels_2, loc="upper right")
...
def save_confusion_matrix(metrics: dict) -> None:
    """
    Plot the confusion matrix for the primary threshold.
    """

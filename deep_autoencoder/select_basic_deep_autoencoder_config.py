"""
Select the deep autoencoder configuration.

This script trains models as temporary candidate models for configuration selection. The final deep model
 is trained by train_final_deep_autoencoder.py.
...
from train_final_deep_autoencoder import (ARTIFACTS_DIR, DATA_PATH, AutoencoderConfig,
                                              config_to_dict, fit_autoencoder,)


TUNING_DIR = ARTIFACTS_DIR / "hyperparameter_search"
TUNING_RESULTS_CSV_PATH = TUNING_DIR / "deep_autoencoder_tuning_results.csv"
TUNING_RESULTS_JSON_PATH = TUNING_DIR / "deep_autoencoder_tuning_results.json"
BEST_CONFIG_PATH = TUNING_DIR / "deep_autoencoder_best_config.json"
...
# grid search
HIDDEN_DIM_OPTIONS = [("deep_width", (48, 24, 18)), ("wider_width", (64, 32, 24)),]
LATENT_DIM_OPTIONS = [4, 8, 12]
...
def evaluate_candidate( model: tf.keras.Model, config: AutoencoderConfig) -> dict[str, float]:
    """
    ...
    # reconstruct error
    normal_errors = reconstruction_errors(model, x_calibration, config.batch_size)
...
def save_selection_outputs( rows: list[dict[str, Any]], best_row: dict[str, Any], best_config: AutoencoderConfig,):
    ...
    best_config_payload = {
        "source": "select_deep_autoencoder_config.py",
...
def select_config() -> dict[str, Any]:
    """
    Train candidate models and save the selected deep configuration.
    """
    ...
    for index, config in enumerate(CANDIDATE_CONFIGS, start=1):
        ...
        detection_result = evaluate_candidate(training_result["model"], config)
    ...
    print("\nBest deep autoencoder configuration:")
